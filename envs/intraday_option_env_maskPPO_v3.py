"""
Intraday Option Environment v3 — Multi-Horizon & Separate CE/PE PnL
====================================================================
Key differences from v2:
  - Multi-horizon blended PnL reward (1-step / 30-step / 120-step)
  - CE and PE unrealized PnL computed separately for reward signal
  - Observation space: 69 features (67 from v2 + ce_unrealized + pe_unrealized)
  - Horizons tuned for weekly episodes (~1,875 steps)

All other features from v2 are preserved:
...
import Mask_PPO_config_v3 as config
...
class IntradayOptionEnvV3(gym.Env):
    """
    ...
    def __init__(self, data_path='data/train.csv', vix_data_path='data/INDIA_VIX.csv',
        ...
        super(IntradayOptionEnvV3, self).__init__()
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(69,), dtype=np.float32)
        ...
        self.pe_pnl_curve = []       # v3: per-leg PE TOTAL PnL history (realized + unrealized)
        self._ce_unrealized = 0.0    # v3: current CE unrealized (for obs)
        self._pe_unrealized = 0.0    # v3: current PE unrealized (for obs)
        self.realized_pnl = 0.0
        self.ce_realized_pnl = 0.0   # v3-fix: per-leg CE realized PnL
        self.pe_realized_pnl = 0.0   # v3-fix: per-leg PE realized PnL
        self.total_trades = 0
    ...
    def reset(self, seed=None, options=None):
        ...
        self.cash = config.INITIAL_CAPITAL
        self.pnl_curve = [0.0]
        self.ce_pnl_curve = [0.0]     # v3: per-leg CE TOTAL PnL history (realized + unrealized)
        self.pe_pnl_curve = [0.0]     # v3: per-leg PE TOTAL PnL history (realized + unrealized)
        self._ce_unrealized = 0.0     # v3: current CE unrealized (for obs)
        self._pe_unrealized = 0.0     # v3: current PE unrealized (for obs)
        self.realized_pnl = 0.0
        self.ce_realized_pnl = 0.0    # v3-fix: per-leg CE realized PnL
    ...
    def step(self, action):
        ...
        if done and self.position_side is not None:
            ...
            self.ce_realized_pnl += pnl_ce
            self.pe_realized_pnl += pnl_pe
        ...
        if self.position_side is not None and self.ce_lots > 0:
            ...
            if self.position_side == 'LONG':
                ce_unrealized = (ce_price - self.entry_price_ce) * self.ce_lots * config.LOT_SIZE
                pe_unrealized = (pe_price - self.entry_price_pe) * self.pe_lots * config.LOT_SIZE
            else:
                ce_unrealized = (self.entry_price_ce - ce_price) * self.ce_lots * config.LOT_SIZE
                pe_unrealized = (self.entry_price_pe - pe_price) * self.pe_lots * config.LOT_SIZE

            # Recalculate margin for SHORT (changes with IV/spot)
            if self.position_side == 'SHORT':
                self._recalculate_total_margin(spot, iv, tte)

        # Store for observation features
        self._ce_unrealized = ce_unrealized
        self._pe_unrealized = pe_unrealized

        # Track per-leg TOTAL PnL curves (v3-fix: realized + unrealized)
        # This ensures smooth transitions on exit/reduce — realized absorbs unrealized
        self.ce_pnl_curve.append(self.ce_realized_pnl + ce_unrealized)
        self.pe_pnl_curve.append(self.pe_realized_pnl + pe_unrealized)

        self.total_pnl = self.realized_pnl + ce_unrealized + pe_unrealized
        ...
        # ── REWARD v3: Multi-Horizon Per-Leg PnL (1-step / 30-step / 120-step) ──
        n = len(self.ce_pnl_curve)
        ce_now = self.ce_pnl_curve[-1]
        pe_now = self.pe_pnl_curve[-1]

        ce_d1   = ce_now - self.ce_pnl_curve[-2]   if n >= 2   else 0.0
        ce_d30  = ce_now - self.ce_pnl_curve[-31]  if n >= 31  else ce_d1
        ce_d120 = ce_now - self.ce_pnl_curve[-121] if n >= 121 else ce_d1

        pe_d1   = pe_now - self.pe_pnl_curve[-2]   if n >= 2   else 0.0
        pe_d30  = pe_now - self.pe_pnl_curve[-31]  if n >= 31  else pe_d1
        pe_d120 = pe_now - self.pe_pnl_curve[-121] if n >= 121 else pe_d1

        ce_term = (config.PNL_W1 * ce_d1 +
                   config.PNL_W30 * ce_d30 / 30 +
                   config.PNL_W120 * ce_d120 / 120) / config.REWARD_NORMALIZER
        pe_term = (config.PNL_W1 * pe_d1 +
                   config.PNL_W30 * pe_d30 / 30 +
        ...
        reward = pnl_term - dd_term + terminal

        # ── INFO ──
        info = {
            'realized_pnl': self.realized_pnl,
            'total_pnl': self.total_pnl,
            'ce_unrealized': ce_unrealized,
            'pe_unrealized': pe_unrealized,
            'pnl_term': pnl_term,
    ...
    def _execute_enter(self, spot, iv, tte, current_time, side):
        ...
        # Transaction costs — split per leg
        ce_txn = self.entry_price_ce * config.LOT_SIZE * config.TRANSACTION_COST_PCT
        pe_txn = self.entry_price_pe * config.LOT_SIZE * config.TRANSACTION_COST_PCT
        self.ce_realized_pnl -= ce_txn
        self.pe_realized_pnl -= pe_txn
        self.realized_pnl -= (ce_txn + pe_txn)
    ...
    def _execute_exit_all(self, spot, iv, tte, current_time):
        ...
        # Transaction costs — split per leg
        ce_txn = ce_price * self.ce_lots * config.LOT_SIZE * config.TRANSACTION_COST_PCT
        pe_txn = pe_price * self.pe_lots * config.LOT_SIZE * config.TRANSACTION_COST_PCT
        total_pnl_close = pnl_ce + pnl_pe - ce_txn - pe_txn

        self.ce_realized_pnl += pnl_ce - ce_txn
        self.pe_realized_pnl += pnl_pe - pe_txn
    ...
    def _execute_add(self, spot, iv, tte, current_time, option_type):
        ...
        if option_type == 'call':
            self.ce_realized_pnl -= txn_cost
        else:
            self.pe_realized_pnl -= txn_cost
    ...
    def _execute_reduce(self, spot, iv, tte, current_time, option_type):
        ...
        if option_type == 'call':
            self.ce_realized_pnl += pnl
        else:
            self.pe_realized_pnl += pnl
    ...
    def _get_observation(self):
        """Calculate all 69 features (v3: +2 per-leg PnL)."""
        ...
        # ── 10. v3 NEW: Per-Leg Unrealized PnL (2) ──
        obs[67] = np.clip(self._ce_unrealized / config.REWARD_NORMALIZER, -5.0, 5.0)
        obs[68] = np.clip(self._pe_unrealized / config.REWARD_NORMALIZER, -5.0, 5.0)
